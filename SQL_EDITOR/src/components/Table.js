...
function Table({ query, headers, rows, csvData }) {
    ...
    <div>
        {query ? (
        <section className="text-sky-400 body-font pl-4">
            <div className="flex w-full justify-between mt-6 lg:mt-0">
            <div className="font-bold text-center py-4 w-40 bg-teal-400 text-white rounded-tl-md rounded-tr-lg">
                Output
            </div>
            <CSVLink
                data={csvData}
                ...
                className="p-2"
            >
                <button className="flex mx-auto text-white bg-teal-400 border-0 py-2 h-11 px-4 focus:outline-none hover:bg-teal-600 rounded text-lg justify-center items-center">
                <svg
